...
export default async function HomePage() {
  ...
  // 1. Fetch initial heroes
  const { docs: initialHeros } = await payload.find({ 
    collection: 'heroship', 
    depth: 1, 
    limit: 100, 
  })

  // 2. Fetch all available tags to pass to the Navbar
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-zinc-100 p-6 mt-16">

      {/* The Interactive Wrapper with TypeScript type assertions */}
      <VaultClient 
        initialHeros={initialHeros as unknown as Hero[]} 
